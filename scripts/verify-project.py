...
def verify(root):
    ...
    check(claude.is_file() and "@AGENTS.md" in claude.read_text(), "CLAUDE.md must import @AGENTS.md")
    ...
    if shared.exists() and ("{{" + "PROJECT_NAME}}") in shared.read_text():
    ...
    for skill in skills:
        ...
        if metadata.is_file():
            explicit_only = bool(re.search(r"^\s+allow_implicit_invocation:\s*false\s*$", metadata.read_text(), re.M))
            check(manual == explicit_only, f"Claude/Codex invocation policy differs: {skill.name}")
        link = root / ".claude/skills" / skill.name
        check(link.is_symlink() and link.resolve() == skill.resolve(), f"Claude entry must link to the project skill: {skill.name}")

    links_root = root / ".claude/skills"
    if links_root.exists():
        for link in links_root.iterdir():
            check(link.name in names and link.is_symlink() and link.exists(), f"Stale or non-shared Claude skill entry: {link.name}")
    ...
    try:
        sources = json.loads((root / "docs/agents/skill-sources.json").read_text())
        ...
        license_body = (skills_root / "security-review/LICENSE").read_text()
    ...
    try:
        ...
        for name in ("harness-reviewer", "harness-researcher"):
            ...
            check(native.is_file() and f"name: {name}" in native.read_text(), f"Missing Claude role: {name}")
